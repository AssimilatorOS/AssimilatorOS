 ...
 * by Andrew Morgan.
 *
 * Updated to work with AssimilatorOS by Gary Greene
 */
...
#define BUFLEN 1024
#ifndef LOGINDEFS
#define LOGINDEFS "/System/cfg/login.defs"
#endif
...
static int _converse(int num_msg, const struct pam_message** msg, struct pam_response** resp, void* appdata_ptr) {
...
static int _authenticate(const char* service, const char* user) {
    ...
    if (err != PAM_SUCCESS) {
        _log_err(LOG_ERR, "pam_start(%s, %s) failed (errno %d)", service, user, err);
        return UNIX_FAILED;
    }

    err = pam_authenticate(pamh, 0);
    if (err != PAM_SUCCESS) {
        _log_err(LOG_ERR, "pam_authenticate(%s, %s): %s", service, user, pam_strerror(pamh, err));
    }
    ...
    if (err == PAM_SUCCESS) {
        ...
        if (err == PAM_SUCCESS) {
            ...
            if (err2 != PAM_SUCCESS) {
                _log_err(LOG_ERR, "pam_setcred(%s, %s): %s", service, user, pam_strerror(pamh, err2));
            }
            /*
            ...
            _log_err(LOG_ERR, "pam_acct_mgmt(%s, %s): %s", service, user, pam_strerror(pamh, err));
        }
    }
...
static int sane_pam_service(const char* name) {
    const char* sp;
    char path[128];
...
static int get_system_fail_delay(void) {
    FILE* fs;
    char buf[BUFLEN];
    long int delay = -1;
    char* s;
...
int main(int argc, char* argv[]) {
    ...
    /*
     * Catch or ignore as many signals as possible.
     */
    setup_signals();
